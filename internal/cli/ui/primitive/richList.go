...
func NewRichList() *RichList {
	return &RichList{
		...
		mainTextColor:                    cview.Styles.PrimaryTextColor,
		selectedBackgroundColor:          cview.Styles.PrimaryTextColor,
		unfocusedSelectedBackgroundColor: cview.Styles.PrimitiveBackgroundColor,
	}
}
...
func (l *RichList) Draw(screen tcell.Screen) {
	...
	for index, item := range l.items {
		...
		if index == l.currentItem {
			...
			cview.Print(screen, []byte(mainText), x, y, width, cview.AlignLeft, l.mainTextColor)

			textWidth := width
			if !l.highlightFullLine {
				if w := cview.TaggedStringWidth(item.MainText); w < textWidth {
					textWidth = w
				}
			}
...
func (l *RichList) InputHandler() func(event *tcell.EventKey, setFocus func(p cview.Primitive)) {
	return l.WrapInputHandler(func(event *tcell.EventKey, setFocus func(p cview.Primitive)) {
